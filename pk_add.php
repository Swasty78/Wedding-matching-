...
$pln_payment= $_POST['pln_payment'];
   ...
   $stmt=$conn->prepare("insert into planner(pln_id,pln_name,phone,pln_payment)
     values(?,?,?,?)");
      $stmt->bind_param("isii",$pln_id,$pln_name,$phone,$pln_payment);
      $stmt->execute();
     }
     $stmt->close();
     $conn->close();
